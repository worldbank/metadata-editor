...
class Analytics_model extends CI_Model
{
    ...
    public function aggregate_api_logs()
    {
        ...
        while ($currentDate < $today) {
            $processed = $this->aggregate_api_logs_day($currentDate, $hasUserTable);
            if ($processed) {
                $daysAggregated++;
    ...
    private function aggregate_api_logs_day($date, $hasUserTable = true)
    {
    ...
    public function archive_and_cleanup()
    {
        ...
        foreach ($months as $month_row) {
            ...
            // Archive to CSV (chunk size: 5000 rows)
            $archived = $this->archive_month($month, 5000);
            if ($archived) {
                $archived_months++;
    ...
    /**
     * Archive a month's data to CSV using chunked processing
     * Processes data in chunks to avoid memory exhaustion
     * 
     * @param string $month Month in YYYY-MM format
     * @param int $chunk_size Number of rows to process per chunk (default: 5000)
     * @return bool Success status
     */
    private function archive_month($month, $chunk_size = 5000)
    {
        $month_start = $month . '-01 00:00:00';
        $month_end = date('Y-m-d 00:00:00', strtotime($month . '-01 +1 month'));
        
        // Validate chunk size
        $chunk_size = max(100, min(50000, (int)$chunk_size)); // Between 100 and 50,000
        ...
        if ($fp === false) {
            log_message('error', 'Analytics: Failed to open archive file - ' . $filename);
            return false;
        }
        ...
        try {
            ...
            while (true) {
                // Get next chunk using cursor-based pagination (more efficient than OFFSET)
                $this->db
                    ->select('*')
                ...
                $chunk = $this->db->get()->result_array();
                ...
                // Write headers on first chunk only
                if (!$headers_written && !empty($chunk)) {
                ...
                if ($total_written % 50000 === 0) {
                    log_message('info', "Analytics: Archived {$total_written} of {$total_count} rows for {$month}");
                }
            }
            ...
            if (!file_exists($filename) || filesize($filename) === 0) {
                log_message('error', 'Analytics: Archive file is empty or missing - ' . $filename);
                return false;
            }
            
            log_message('info', "Analytics: Successfully archived {$total_written} rows for {$month} to {$filename}");
            return true;
            ...
            log_message('error', 'Analytics: Archive failed for ' . $month . ' - ' . $e->getMessage());
            return false;
        }
    }
