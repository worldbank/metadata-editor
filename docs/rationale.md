...
- **UNSTRUCTURED DATA**
   ...
   - **Video recordings**: Speech-to-text algorithms can automatically generate transcriptions of audio and video recordings. They can thus be considered as data in the same way we consider text as data. The Metadata Editor uses a specific schema made of elements from the Dublin Core and from the videoObject standard from schema.org.
